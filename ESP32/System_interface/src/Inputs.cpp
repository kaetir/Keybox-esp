...
int Inputs::get_input()
{
    ...
    for (int i = 0; i < input_pin.size(); i++)
    {
        ...
        if (input_val[input_val.size() - 1] <= THRESHOLD_VAL && has_input == false) //ONE INPUT WAS DETECTED
        {
            has_input = true;
        }
        else if (input_val[input_val.size() - 1] <= THRESHOLD_VAL && has_input == true) //MULTIPLE INPUT DETECTED
        {
            return -2;
        }
    }
    ...
    {
        while (this->has_input() == true) //WAIT FOR THE USER TO RELEASE THE INPUT/BUTTON
        {
            delay(100);
        }
