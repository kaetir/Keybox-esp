...
Controller::Controller()
{
    ...
    if (conf.size() == 0 || this->isWalletCreated == false) //FIRST BOOT DETECTED
    {
...
void Controller::load_menu(std::string menu_name)
{
    ...
    this->menu_lines = this->model->read(SPIFFS, menu_name, "txt", this->language);

    //GETTING THE MENU STRUCTURE
    std::vector<std::string> struct_tmp = this->model->read(SPIFFS, menu_name, "struct", this->language);
    std::vector<std::string> tmp_vect;
        ...
        if (this->buffer[i] == this->cursor_position) //CURSOR POSITION FOUND IN THE MENU BUFFER
        {
            if (std::binary_search(this->line_number_of_choices.begin(), this->line_number_of_choices.end(), this->buffer[i]) == true) //THE CURSOR IS ON AN ENTRY
            {
                ...
                else if (funct == "First_boot") //SET THE LANGUAGE AT THE FIRST BOOT
                {
                    ...
                    {
                        //TODO: LANCER LE SERVER ET WIFI HOTSPOT
                    }
                    else
                    {
                        this->first_boot = false;
