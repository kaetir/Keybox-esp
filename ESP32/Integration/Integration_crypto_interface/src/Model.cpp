...
{
  if (this->wallet->addAccount(name, user, pwd) == true) //ADD AN ACCOUNT TO THE LIST
  {
...
{
  if (this->wallet->changePwd(account_index, password) == true) //MODIFY THE PASSWORD OF AN ACCOUNT
  {
...
{
  if (this->wallet->changeUsername(account_index, user) == true) //MODIFY THE USERNAME OF AN ACCOUNT
  {
...
bool Model::logout()
{
  ...
  this->wallet->locked();
  this->locker.clear();
  //TODO: SUPPRIMER/ETEINDRE LE SERVEUR
  return true;
}
