...
bool Wallet::isWalletcreated() {
  if (this->masterUser == "" || this->mainkey[0] == "" ||
      this->hashWord == "") {
    return false;
  }
  return true;
...
void Wallet::createWallet(std::string masterUser, std::string pwd) {
  ...
  */
  ...
  std::stringstream ss;
  for (int i = 0; i < 32; ++i) ss << HEX << (int)shaResult[i];
...
bool Wallet::addAccount(std::string nameAccount, std::string username,
                        std::string pwd, int l) {
  /*
  ...
  */
  if (this->lock == false) {
    ...
    // in order to manipulate string outside wallet we need to transform a
    // string in a char
    int n = 16;
    ...
    std::string tpwd1(reinterpret_cast<const char*>(ttest),
                      strlen((char*)ttest));
    std::string tpwd2(reinterpret_cast<const char*>(t2test),
                      strlen((char*)t2test));
