...
void Wallet::createWallet(std::string masterUser, std::string masterWord)
{
    ...
    */
    ...
    std::string mainkey_rng = random_string(32)
        mainkey
        = mdp_crypt(main_password_hash, mainkey_rng)
            concombre
        = mdp_crypt(mainkey_rng, "concombre") unsigned char main[32];

    char* rkey = new char[masterWord.size() + 1];
    std::copy(masterWord.begin(), masterWord.end(), ca);
    ca[masterWord.size()] = '\0';
    unsigned char shaResult[32];
    std::vector<char> cstr(masterWord.c_str(), masterWord.c_str() + masterWord.size() + 1);

    pwd_crypt(shaResult, mainkey_rng.c_str(), main);
    std::string mainkey(reinterpret_cast<const char*>(shaResult), 32);
    unsigned char check[32];
    char* key = (char*)"concombre";
    pwd_crypt(shaResult, key, main);
}
